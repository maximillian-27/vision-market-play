    ...
    <div className="space-y-6">
      <Tabs defaultValue="active" className="space-y-4">
        <TabsList className="bg-muted/50 p-1">
          <TabsTrigger value="active" className="data-[state=active]:bg-background gap-2">
            <Gift className="h-4 w-4" /> Active Bonuses
          </TabsTrigger>
          <TabsTrigger value="create" className="data-[state=active]:bg-background gap-2">
          ...
          <TabsTrigger value="loyalty" className="data-[state=active]:bg-background gap-2">
            <Trophy className="h-4 w-4" /> Loyalty Tiers
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="border-border/40">
              <CardContent className="p-4">
                ...
                <p className="text-2xl font-bold">{activeBonuses.filter(b => b.active).length}</p>
              </CardContent>
            </Card>
            <Card className="border-border/40">
              <CardContent className="p-4">
                <p className="text-sm text-muted-foreground mb-1">Total Claims</p>
                <p className="text-2xl font-bold">{activeBonuses.reduce((a, b) => a + b.used, 0).toLocaleString()}</p>
              </CardContent>
            </Card>
            <Card className="border-border/40 bg-success/5">
              <CardContent className="p-4">
                <p className="text-sm text-muted-foreground mb-1">Total Budget</p>
                <p className="text-2xl font-bold">${(activeBonuses.reduce((a, b) => a + b.budget, 0) / 1000).toFixed(0)}K</p>
              </CardContent>
            </Card>
            <Card className="border-border/40 bg-warning/5">
              <CardContent className="p-4">
                <p className="text-sm text-muted-foreground mb-1">Budget Spent</p>
                <p className="text-2xl font-bold">${(activeBonuses.reduce((a, b) => a + b.spent, 0) / 1000).toFixed(0)}K</p>
              </CardContent>
            </Card>
          </div>
                ...
                <tbody>
                  {activeBonuses.map((b) => (
                    <tr key={b.id} className="border-b border-border/20 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="p-4"><Switch defaultChecked={b.active} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input placeholder="e.g. Summer Special" className="mt-1" />
                </div>
                <div>
                  <Label>Type</Label>
                ...
                <div>
                  <Label>Value</Label>
                  <Input placeholder="e.g. 100% or $10" className="mt-1" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input placeholder="e.g. 50000" type="number" className="mt-1" />
                </div>
                <div>
            ...
            <Card className="border-border/40">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold">Super Bowl Special</span>
                  <Badge className="text-xs">Active</Badge>
                </div>
                <p className="text-sm text-muted-foreground mb-3">Bet $50 on any Super Bowl market, get $10 free bet</p>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Claims: 890</span>
                  <span className="text-muted-foreground">Ends: Mar 15</span>
                </div>
              </CardContent>
            </Card>
            <Card className="border-border/40">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="font-semibold">Crypto Markets Boost</span>
                  <Badge className="text-xs">Active</Badge>
                </div>
                <p className="text-sm text-muted-foreground mb-3">2x points on all crypto market trades this week</p>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Claims: 2,340</span>
                  <span className="text-muted-foreground">Ends: Feb 28</span>
                </div>
              </CardContent>
            </Card>
          </div>
