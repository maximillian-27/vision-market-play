...
import { Search, MoreHorizontal, Eye, Download, ArrowUpRight, ArrowDownRight, Receipt, Shield, ChevronLeft, ChevronRight, Wallet, Percent, AlertTriangle, CheckCircle, XCircle, Flag } from "lucide-react";
...
const wallets = [
  { name: "Hot Wallet (ETH)", address: "0x742d35Cc6634C0532925a3b844Bc9e7595f", balance: "$2,450,000", chain: "Ethereum", inflow24h: "+$120K", outflow24h: "-$45K", lastTx: "8 min ago" },
  { name: "Hot Wallet (BTC)", address: "bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh", balance: "$1,890,000", chain: "Bitcoin", inflow24h: "+$312K", outflow24h: "-$89K", lastTx: "25 min ago" },
  { name: "Cold Storage", address: "0x89205A3A3b2A69De6Dbf7f01ED13B2108B2c43e7", balance: "$12,500,000", chain: "Multi-chain", inflow24h: "+$0", outflow24h: "-$0", lastTx: "3 days ago" },
  { name: "USDT Reserve", address: "TN3W4H6rK2ce4vX9YnFQHwKENnHjoxb3m9", balance: "$5,200,000", chain: "Tron", inflow24h: "+$380K", outflow24h: "-$156K", lastTx: "1h ago" },
];
...
export const AdminTransactions = () => {
  ...
  const filteredTransactions = transactions.filter((txn) => {
    ...
    const matchesStatus = statusFilter === "all" || txn.status.toLowerCase().replace(" ", "_") === statusFilter;
                        ...
                        <td className="p-4 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="bg-popover">
                              ...
                              <DropdownMenuItem className="gap-2" onClick={() => toast.success(`Transaction ${txn.id} flagged for review`)}><Flag className="h-4 w-4" /> Flag</DropdownMenuItem>
              ...
              <Card key={w.name} className="border-border/40">
                <CardContent className="p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span className="text-muted-foreground">24h In</span><span className="text-success">{w.inflow24h}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">24h Out</span><span className="text-destructive">{w.outflow24h}</span></div>
                    <div className="flex justify-between"><span className="text-muted-foreground">Last Tx</span><span>{w.lastTx}</span></div>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <Button size="sm" variant="outline" className="flex-1" onClick={() => toast(`Initiating transfer from ${w.name}`)}>Transfer</Button>
                    <Button size="sm" variant="outline" className="flex-1 text-destructive" onClick={() => toast.success(`${w.name} frozen`)}> Freeze</Button>
                  </div>
                </CardContent>
              </Card>
            ))}
